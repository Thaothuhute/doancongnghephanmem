...
namespace thulan2
{
    public partial class fReportThanhtoan : Form
    {
        public fReportThanhtoan()
        {
            InitializeComponent();
        }

        private void fReportThanhtoan_Load(object sender, EventArgs e)
        {
            ...
            using (phongtroDBContext context = new phongtroDBContext())
            {
                ...
                List<PHIEUTHANHTOAN> pttoans = context.PHIEUTHANHTOANs.ToList().Where(p => p.Trangthai == 1).ToList();
